"""nlegs_flat: 窄本体平地行走, 全量展开配置(不继承 legs/legs_dr/nlegs 任何 env cfg)。

复刻 nlegs run 2026-08-25_20-27-40 的最终生效配置(以其 params/env.yaml 为准):
- legs 基础配置 + _apply_nlegs 覆盖(资产/质量/reset/奖励权重) + _apply_ankle_dr(踝执行器拆组+踝增益 DR)
  的合并结果, 所有值就地写死。
- 机器人定义(含踝执行器拆组后的三组驱动器参数)在 legs_rl_lab.assets.nlegs.nlegs 的 NLEGS_CFG 里;
  踝增益 DR(randomize_ankle_gains)留在本文件 EventCfg。
- 有意不含 legs_dr 的全局 DR 四件套(link_mass/base_com/actuator_gains/joint_params):
  该 run 实际继承的是非 DR 版 RobotEnvCfg, 训练时这四项不存在(env.yaml 证实, events 仅 7 项)。
- 用到的 mdp 函数在 legs_rl_lab.tasks.nlegs_task.mdp 下(isaaclab 内置项仍从 isaaclab.envs.mdp 星导入)。
"""
...
@configclass
class EventCfg:
    """事件项(共 7 项, 与 run 2026-08-25_20-27-40 的 env.yaml 一致; 无全局 DR 四件套)。"""
    ...
    # 下限 kp scale 0.7 远在发散区(kp≈10-15)之上。
...
@configclass
class RewardsCfg:
    """Reward terms for the MDP (nlegs 覆盖值已就地写死)。"""
    ...
    # -- robot
    base_height = RewTerm(func=mdp.base_height_l2, weight=-5, params={"target_height": 0.58})
...
@configclass
class FlatPlayEnvCfg(FlatEnvCfg):
    def __post_init__(self):
        super().__post_init__()
