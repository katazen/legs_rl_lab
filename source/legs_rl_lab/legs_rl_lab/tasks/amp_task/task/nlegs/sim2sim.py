"""nlegs + AMP 走路策略的 sim2sim(MuJoCo)验证(自包含, 不依赖 legs 目录)。

AMP 任务的**策略推理**与 nlegs 完全同构:同一窄本体(legs_narrow)+同一场景 xml,
策略观测组 7 项、顺序、history 与 nlegs 一致(判别器仅训练用, 部署只跑 actor),
obs 布局/关节序/PD/DCMotor 参数照常从训练导出的 deploy.yaml 读取。故本文件与
legs_task 的 nlegs sim2sim 逻辑一致, 仅把日志实验名指向 `nlegs_amp`。

用法:
    # 先 play 导出策略, 使 logs/rsl_rl/nlegs_amp/<RUN>/exported/policy.pt 存在
    python source/legs_rl_lab/legs_rl_lab/tasks/amp_task/task/nlegs/sim2sim.py
键盘控速(数字键盘): 8/2 vx±, 4/6 vy±, 7/9 yaw±。
"""
import os
import time
import types
