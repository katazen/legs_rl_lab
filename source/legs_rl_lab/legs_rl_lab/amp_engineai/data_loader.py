"""AMP 专家数据加载器 (照搬 engineai AMP_data_loader.py)。

读 engineai 格式 npz(键: fps, joint_pos, joint_vel, body_pos_w, body_quat_w,
body_lin_vel_w, body_ang_vel_w); 把各 body 位姿转到 anchor(body 0 = base) 相对系,
base_lin_vel_b = body 0 的线速度(转到 base 系)。

mini_batch_generator 逐帧特征 = [joint_pos*9, base_lin_vel_b*7]; 用 history_offsets
(-(H-1)..0) 堆叠 H 帧, 最后一帧对齐当前 index -> 输出 (batch, H*frame_dim)。
"""
...
class AMPDataLoader:
    def __init__(
        ...
        for i in range(self.time_step_total):
            ...
            # 速度转到 anchor 系: 用 anchor 四元数的逆, 按 body 广播
    ...
    def mini_batch_generator(self, num_mini_batches, num_epoches) -> Iterator[torch.Tensor]:
        ...
        # history 由旧到新; offsets 从负到 0, 最后一帧为"当前"
        ...
        for _ in range(num_epoches):
            for i in range(0, num_samples, batch_size):
                ...
                frame_features = []
                # history 方向: 旧 -> 新 (最后一帧对齐 batch_indices)
