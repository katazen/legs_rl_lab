"""AMP 判别器 (照搬 engineai AMP_discriminator.py)。

前馈网络, 输入是 history_length 帧堆叠的 frame 特征; 可选逐帧 EmpiricalNormalization。
LSGAN: forward 输出标量分数; get_amp_reward = clamp(1 - 0.25*(d-1)^2, min=0);
compute_grad_pen 把梯度范数压到 0。
"""
...
class Discriminator(nn.Module):
    """Simple feedforward discriminator for Adversarial Motion Prior."""

    def __init__(
        self,
        input_dim_per_frame: int = 15,
        input_history_length: int = 5,
        hidden_dims: list[int] = [512, 256, 128],
        activation: str = "relu",
        feature_normalization: bool = False,  # True -> 用 EmpiricalNormalization 逐帧归一化
        device: str = "cpu",
    ):
        super().__init__()
    ...
    def normalize_input(self, x):
        if self.feature_normalization:
            # 逐帧切片归一化再拼回, 避免对 leaf tensor 做 in-place
    ...
    def forward(self, x):
        assert self.history_length * self.frame_size == x.shape[1], (
            f"Input feature dimension {x.shape[1]} does not match expected size "
            f"{self.history_length * self.frame_size}"
        )
        if self.feature_normalization:
