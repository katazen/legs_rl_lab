"""AMPPPO (engineai 风格): 在 rsl_rl 5.0.1 官方 PPO 上叠加 history-window AMP。

对照本仓 amp/amp_ppo.py 的差异(完全照搬 engineai):
  - style reward 加性: total = task + 0.01*style_reward_weight*disc.get_amp_reward(obs["amp"])。
  - 判别器每 4 次 PPO update 训练一次; policy 样本直接取 storage 的 batch.observations["amp"]
    (history-window 已在 env 侧堆好), 无 replay buffer / 无 (s,s') 记录。
  - 专家样本来自 AMPDataLoader.mini_batch_generator; LSGAN expert->+1, policy->-1,
    grad penalty(lambda=10)。判别器用独立 Adam(lr=1e-4)。

接线: agent cfg 顶层暴露 frame_dim/frame_length/discriminator_hidden_dims/
frame_normalization/dataset_path; construct_algorithm 读这些顶层键构建判别器 + 专家数据,
再复用 PPO.construct_algorithm 造 actor/critic/storage。
"""
...
class AMPPPO(PPO):
    ...
    def construct_algorithm(obs, env, cfg: dict, device: str) -> "AMPPPO":
        # 顶层 amp 配置注入 cfg["algorithm"], 供 PPO.construct_algorithm 以 **kwargs 传给 __init__
    ...
    def update(self):  # noqa: C901
        ...
        if self.ppo_update_counter % 4 == 0:
            ...
            mean_amp_loss = 0
            reference_data_generator = self.discriminator_data_loader.mini_batch_generator(
