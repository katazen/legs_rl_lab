...
export LEGS_RL_LAB_PATH="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
...
_legs_rl_lab_python_argcomplete_wrapper() {
    local IFS=$'\013'
    ...
    COMPREPLY=( $(IFS="$IFS" \
                    ...
                    ${python_exe} ${LEGS_RL_LAB_PATH}/scripts/rsl_rl/train.py 8>&1 9>&2 1>/dev/null 2>/dev/null) )
}
complete -o nospace -F _legs_rl_lab_python_argcomplete_wrapper "./legs_rl_lab.sh"


_legs_setup_conda_env() {
    ...
    printf '%s\n' '#!/usr/bin/env bash' '' \
        ...
        '' \
        '# for legs_rl_lab' \
        'source '${LEGS_RL_LAB_PATH}'/legs_rl_lab.sh' \
...
case "$1" in
    -i|--install)
        ...
        pip install -e ${LEGS_RL_LAB_PATH}/source/legs_rl_lab/
        _legs_setup_conda_env
        activate-global-python-argcomplete
        ;;
    -l|--list)
        shift
        ${python_exe} ${LEGS_RL_LAB_PATH}/scripts/list_envs.py "$@"
        ;;
    -p|--play)
        shift
        ${python_exe} ${LEGS_RL_LAB_PATH}/scripts/rsl_rl/play.py "$@"
        ;;
    -t|--train)
        shift
        ${python_exe} ${LEGS_RL_LAB_PATH}/scripts/rsl_rl/train.py --headless "$@"
        ;;
    *) # unknown option
        ;;
esac
