...
class RL_real(Node):
    ...
    def _tick(self):
        ...
        m = Float64MultiArray()
        m.data = self.target_real.tolist()
        self.pub.publish(m)
    ...
    def _on_joy(self, msg):
        ...
        self.cmd[0] = ax(1) * self.cmd_clip[0]       # 左摇杆纵 = vx
        self.cmd[1] = ax(0) * self.cmd_clip[1]       # 左摇杆横 = vy
        self.cmd[2] = ax(2) * self.cmd_clip[2]       # 右摇杆横 = yaw
    ...
    def _read_keys(self):
        while True:
            ...
            if not ch:
                break
            if ch in "wW":
                self.cmd[0] = min(self.kb_max[0], self.cmd[0] + self.kb_step)
            elif ch in "sS":
                self.cmd[0] = max(-self.kb_max[0], self.cmd[0] - self.kb_step)
            elif ch in "aA":
                self.cmd[1] = min(self.kb_max[1], self.cmd[1] + self.kb_step)
            elif ch in "dD":
                self.cmd[1] = max(-self.kb_max[1], self.cmd[1] - self.kb_step)
            elif ch in "qQ":
                self.cmd[2] = min(self.kb_max[2], self.cmd[2] + self.kb_step)
            elif ch in "eE":
                self.cmd[2] = max(-self.kb_max[2], self.cmd[2] - self.kb_step)
            elif ch == " ":
                self.cmd[:] = 0.0
            elif ch in "pP":
