...
parser.add_argument("--delay-steps", type=int, default=None, help="固定 DelayedDCMotor 延迟步数(5ms/步)")
...
def main():
    ...
    if args.delay_steps is not None:
        act_cfg = cfg.actuators["delayed_dcmotor"]
        act_cfg.min_delay = act_cfg.max_delay = args.delay_steps
        print(f"[override] delayed_dcmotor delay -> {args.delay_steps} steps ({args.delay_steps * SIM_DT * 1000:.0f}ms)")
    ...
    if args.replay_csv:                       # 用实机 cmd 文件的目标驱动(替代 chirp)
        ...
        rep_phase = np.array([x.get("phase", "") for x in rr])
    ...
    for i in range(n+1):
        ...
        if rep_t is not None:
            ...
            phase = rep_phase[max(0, np.searchsorted(rep_t, tt, side="right") - 1)]
        else:
        ...
        rows.append((tt, phase, finst, tgt, q, v))
        if not app.is_running(): break
    os.makedirs(os.path.dirname(args.out), exist_ok=True)
    with open(args.out,"w") as f:
        f.write("t,phase,finst,target,q,v\n")
        for t, phase, finst, target, q, v in rows:
            f.write(f"{t:.6f},{phase},{finst:.6f},{target:.6f},{q:.6f},{v:.6f}\n")
