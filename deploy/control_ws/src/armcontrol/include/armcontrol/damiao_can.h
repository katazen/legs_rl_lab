...
#pragma pack()
    ...
    Limit_param limit_param[Num_Of_Motor]=
            {
                    {12.5, 45, 30 }, // DM4310  (本机固件实测 PMAX/VMAX/TMAX=12.5/45/30)
                    {12.5, 50, 10 }, // DM4310_48V
                    {12.5, 45, 30 }, // DM4340  (本机固件实测 12.5/45/30)
    ...
    public:
        ...
        // 新增：接收并保存错误码（4bits）
        void receive_error(uint8_t err)
        {
            this->err_code = err & 0x0F;
            if (this->err_code > 0x01) // 如果有错误码，可以在这里添加日志打印或其他处理
            {
                std::cerr << "Motor ID " << std::hex << this->Slave_id << " Error: " << Motor::error_code_to_string(this->err_code) << std::endl;
            }
        }

        // 新增：获取错误码
        uint8_t GetErrCode() const { return this->err_code; }
    ...
    public:
         ...
         *
         */
        Gripper(uint8_t node_id)
                : node_id(node_id) {
        }

        void receive_data(uint8_t fault_code, uint8_t state, uint8_t pos, uint8_t vel, uint8_t force)
        {
         ...
         * @return MasterID
         */
        uint8_t GetNodeId() const { return this->node_id; }

        /*
         * @brief get motor position 获取电机位置
         * @return motor position 电机位置
         */
        uint8_t GetPosition() const { return this->pos; }

        /*
         * @brief get motor velocity 获取电机速度
         * @return motor velocity 电机速度
         */
        uint8_t GetVelocity() const { return this->vel; }

        /*
         * @brief get torque of the motor  获取电机实际输出扭矩
         * @return motor torque 电机实际输出扭矩
         */
        uint8_t GetForce() const { return this->force; }

        uint8_t GetFaultCode() const { return this->fault_code; }

        uint8_t GetState() const { return this->state; }
    };

    class GripperJoystick
    {
    private:
        /* data */
        uint8_t node_id;
        ...
        uint16_t joy;
    public:
        GripperJoystick(uint8_t node_id)
                : node_id(node_id) {
        }

        void receive_data(uint8_t sw1, uint8_t sw2, uint8_t sw3, uint8_t sw4, uint8_t led, uint16_t joy)
        {
            this->sw1 = sw1;
            this->sw2 = sw2;
            this->sw3 = sw3;
        ...
        uint8_t GetNodeId() const { return this->node_id; }
        uint8_t GetSw1() const { return this->sw1; }
        uint8_t GetSw2() const { return this->sw2; }
        uint8_t GetSw3() const { return this->sw3; }
        uint8_t GetSw4() const { return this->sw4; }
        uint8_t GetLed() const { return this->led; }
        u_int16_t GetJoy() const { return this->joy; }
    }; 
 ...
 */
    ...
    public:
        ...
        /*
        * @brief 定义电机控制对象
        * @param can_socket_interface socketCan对象
        * 默认串口为Can0
        */
        // Motor_Control(std::shared_ptr<CanSocket> can_socket_interface = nullptr)
        //         : can_socket_interface_(std::move(can_socket_interface))    
        // {
        //     if (can_socket_interface_ == nullptr) {
        //         can_socket_interface_ = std::make_shared<CanSocket>("can0");
        //     }
        //     can_socket_interface_->Init();
        //     can_socket_interface_->setRecvCallback([&](uint32_t can_id, const uint8_t* data, uint8_t len) {
        //         if (len == 8) {
        //             this->receive_can_data(can_id, data);
        //         }
        //     });
        // }

        Motor_Control(const std::string& bus_name): bus_name_(bus_name)
        {
        }
        ...
        bool init(void)
        {
            can_socket_interface_ = std::make_shared<SocketCAN>();
            int thread_priority=95;
            if (can_socket_interface_->open(bus_name_, std::bind(&Motor_Control::canframeCallback, this, std::placeholders::_1), thread_priority)) {
                return true;
            } else {
                return false;
            }
        ...
        {
            std::lock_guard<std::mutex> guard(canframe_mutex_);
            // RCLCPP_INFO(rclcpp::get_logger("CanSocket"), "[Reader] %s : 0x%X, DLC: %d", bus_name_.c_str(), frame.can_id, frame.can_dlc);
            
            if (frame.can_dlc == 8) {
                this->receive_can_data(frame.can_id, frame.data);
            }
        }
        ...
        */
        ...
        void refresh_motor_status(const Motor& motor)
        {
            ...
            can_socket_interface_->sendFrame(send_data.canId, send_data.data, 8);
            // this->receive();
        }
        /*
        ...
        */
        ...
        void clear_err(const Motor& motor) {
        ...
        */
        ...
        void receive()
        {
            ...
            if(receive_data.CMD == 0x11 && receive_data.frameEnd == 0x55) // receive success
            {
                ...
                if(gripper != nullptr && receive_data.canId == gripper->GetNodeId())
                {
                    // gripper data
                    ...
                    uint8_t pos = data[2];
                    uint8_t vel = data[3];
                    uint8_t force = data[4];
                    gripper->receive_data(fault_code, state, pos, vel, force);
                    return;
                }
                else if(gripper_joystick != nullptr && receive_data.canId == gripper_joystick->GetNodeId())
                {
                    // gripper joystick data
                    auto & data = receive_data.canData;
                    uint8_t led = data[0];
                    uint8_t sw1 = data[3];
                    uint8_t sw2 = data[4];
                    uint8_t sw3 = data[5];
                    uint8_t sw4 = data[6];
                    uint16_t joy = (data[1] << 8) | data[2];
                    gripper_joystick->receive_data(sw1, sw2, sw3, sw4, led, joy);
                    return;
                }

                auto & data = receive_data.canData;

                // 解析错误码：data[0] 的高 4 位为 ERR（低 4 位可能是从机ID）
                uint8_t err_nibble = (data[0] >> 4) & 0x0F;
                auto m = motors[receive_data.canId];
                m->receive_error(err_nibble);
            ...
            else if (receive_data.CMD == 0xEE) // communication error
            {
                ...
                // 保存/更新错误码
                m->receive_error(err_nibble);
                std::string err_str = Motor::error_code_to_string(err_nibble);
                std::cerr << "Motor ID " << std::hex << receive_data.canId << " Error: " << err_str << std::endl;
            }
        }
         ...
         * @description 函数库内部调用,用于解算电机can线返回的电机参数
         */
        void receive_can_data(uint32_t can_id, const uint8_t* can_data)
        {
            static auto uint_to_float = [](uint16_t x, float xmin, float xmax, uint8_t bits) -> float {
                float span = xmax - xmin;
                float data_norm = float(x) / ((1 << bits) - 1);
            ...
            if(gripper != nullptr && can_id == gripper->GetNodeId())
            {
                // gripper data
                auto & data = can_data;
                uint8_t fault_code = data[0];
            ...
            else if(gripper_joystick != nullptr && can_id == gripper_joystick->GetNodeId())
            {
                // gripper joystick data
                auto & data = can_data;
                uint8_t led = data[0];
            ...
            else if(can_id == 0x7FF) // param receive
            {
                auto & data = can_data;
                if(data[2]==0x33 or data[2]==0x55)
                {
            ...
            // 解析错误码：data[0] 的高 4 位为 ERR（低 4 位可能是从机ID）
            uint8_t err_nibble = (data[0] >> 4) & 0x0F;
            auto m = motors[can_id];
            m->receive_error(err_nibble);

            uint16_t q_uint = (uint16_t(data[1]) << 8) | data[2];
            uint16_t dq_uint = (uint16_t(data[3]) << 4) | (data[4] >> 4);
            uint16_t tau_uint = (uint16_t(data[4] & 0xf) << 8) | data[5];
            if(can_id != 0x00)   //make sure the motor id is not 0x00
            {
        ...
        void addGripper(Gripper *DM_Gripper)
        {
            gripper = DM_Gripper;
        }

        void addGripperJoystick(GripperJoystick *DM_GripperJoystick)
        {
            gripper_joystick = DM_GripperJoystick;
        }

        void control_gripper(const Gripper* gripper, uint8_t pos, uint8_t vel, uint8_t force)
        {
            std::array<uint8_t, 8> data_buf = {0x00, pos, vel, force, 0x7F, 0x7F, 0x00, 0x00};
            send_data.modify(gripper->GetNodeId(), data_buf.data());
            // serial_->send((uint8_t*)&send_data, sizeof(can_send_frame));
            can_socket_interface_->sendFrame(send_data.canId, send_data.data, 8);
            // this->receive();
        }
        ...
        void control_gripper_joystick(const GripperJoystick* gripper_joystick, uint8_t led)
        {
            std::array<uint8_t, 8> data_buf = {led, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
            send_data.modify(gripper_joystick->GetNodeId(), data_buf.data());
            // serial_->send((uint8_t*)&send_data, sizeof(can_send_frame));
            can_socket_interface_->sendFrame(send_data.canId, send_data.data, 8);
            // this->receive();
        }
    ...
    private:
        ...
        Gripper *gripper = nullptr; //单个夹爪
        GripperJoystick *gripper_joystick = nullptr; //单个夹爪手柄
        // SerialPort::SharedPtr serial_;  //serial port
        // std::shared_ptr<CanSocket> can_socket_interface_;
        std::shared_ptr<SocketCAN> can_socket_interface_;
        mutable std::mutex canframe_mutex_;
