...
#pragma pack()
    ...
    public:
        ...
        void receive_data(uint8_t sw1, uint8_t sw2, uint8_t sw3, uint8_t sw4, uint8_t sw5, uint8_t led, uint16_t joy, uint16_t adc1)
        {
            this->sw1 = sw1;
            this->sw2 = sw2;
            this->sw3 = sw3;
        ...
        uint8_t GetSw5() const { return this->sw5; }
        uint8_t GetLed() const { return this->led; }
        uint16_t GetJoy() const { return this->joy; }
        uint16_t GetAdc1() const { return this->adc1; }
    };
        ...
        */
        void control_mit(Motor &DM_Motor, float kp, float kd, float q, float dq, float tau)
        {
            ...
            if(motors.find(id) == motors.end())
            {
                ...
                printf("Motor_Control id not found\n");
                return;
            }
            auto& m = motors[id];
        ...
        void receive()
        {
            ...
            if(receive_data.CMD == 0x11 && receive_data.frameEnd == 0x55) // receive success
            {
                ...
                {
                //     夹爪ADC值  滑轮ADC值   按键                   LED状态  预留
                //     2 Bytes   2 Bytes    1 Byte                1 Byte  2 Bytes
                //     0-3920    0-4064     bit0-bit4对应按键1-5
                    ...
                    // uint8_t sw3 = data[5];
                    // uint8_t sw4 = data[6];
                    uint16_t adc1 = (data[0] << 8) | data[1];
                    uint16_t joy = (data[2] << 8) | data[3];
                    uint8_t sw1 = (data[4] >> 0) & 0x01;
                    uint8_t sw2 = (data[4] >> 1) & 0x01;
                    uint8_t sw3 = (data[4] >> 2) & 0x01;
                    uint8_t sw4 = (data[4] >> 3) & 0x01;
                    uint8_t sw5 = (data[4] >> 4) & 0x01;
                    uint8_t led = data[5];
                    // printf("joy:%d adc1:%d data[0]:%x, data[1]:%x, data[2]:%x, data[3]:%x, data[4]:%x, data[5]:%x \n",joy , adc1, data[0], data[1],data[2],data[3],data[4], data[5]);
                    gripper_joystick->receive_data(sw1, sw2, sw3, sw4, sw5, led, joy, adc1);
                    return;
                }
